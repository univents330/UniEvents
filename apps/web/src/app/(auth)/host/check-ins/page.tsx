...
import { useCheckIns, useCreateCheckIn, useDeleteCheckIn } from "@/features/check-ins";
import { useEvents } from "@/features/events";
import { useValidatePass } from "@/features/passes";
...
export default function HostCheckInsPage() {
	...
	const attendeesQuery = useAttendees(
		manualEventId
			? { eventId: manualEventId, page: 1, limit: 100, sortBy: "name", sortOrder: "asc" }
			: undefined,
	);
	...
	const attendeeIds = useMemo(
		() => [...new Set(visibleCheckIns.map((c) => c.attendeeId))],
		[visibleCheckIns],
	);
	// We resolve names by fetching attendees for each unique attendeeId via the list endpoint
	// Since the server scopes by role, we just fetch all and build a map
	...
	async function handleQrCheckIn() {
		...
		const pass = await validatePass.mutateAsync({ code: qrCode.trim(), eventId: qrEventId });
		await createCheckIn.mutateAsync({
		...
		<div className="space-y-6">
			<div className="flex items-start justify-between">
				<div>
					...
					<p className="mt-2 text-slate-600">Manage and track event check-ins.</p>
				</div>
				<div className="flex gap-2">
					<button
						type="button"
						onClick={() => { setShowScanner(true); setQrEventId(""); setQrCode(""); }}
						className="flex items-center gap-2 rounded-lg border border-[#030370] px-4 py-2 font-semibold text-sm text-[#030370] transition hover:bg-[#f0f4ff]"
					>
					...
					<button
						type="button"
						onClick={() => { setShowManual(true); setManualEventId(""); setManualAttendeeId(""); }}
					...
					<table className="w-full">
						<thead>
							<tr className="border-slate-200 border-b bg-slate-50">
								<th className="px-4 py-3 text-left font-semibold text-slate-900 text-sm">Attendee</th>
								<th className="px-4 py-3 text-left font-semibold text-slate-900 text-sm">Event</th>
								<th className="px-4 py-3 text-left font-semibold text-slate-900 text-sm">Method</th>
								<th className="px-4 py-3 text-left font-semibold text-slate-900 text-sm">Timestamp</th>
						...
						<tbody className="divide-y divide-slate-200">
							{checkInsQuery.isLoading ? (
								<tr>
									<td className="px-4 py-6 text-slate-600" colSpan={5}>Loading check-ins...</td>
								</tr>
							) : filteredCheckIns.length === 0 ? (
								<tr>
									<td className="px-4 py-6 text-slate-600" colSpan={5}>No check-ins found.</td>
								</tr>
							) : (
								filteredCheckIns.map((checkIn) => (
									<tr key={checkIn.id} className="hover:bg-slate-50">
										<td className="px-4 py-3 font-medium text-slate-900">
											{attendeeNameById.get(checkIn.attendeeId) ?? checkIn.attendeeId}
										</td>
										<td className="px-4 py-3 text-slate-700">
											{eventNameById.get(checkIn.eventId) ?? checkIn.eventId}
										</td>
										<td className="px-4 py-3">
											<span className={`rounded-full px-2 py-1 font-semibold text-xs ${
									...
									p === "..." ? (
										...
										<button
											...
											className={`min-w-[32px] rounded-lg px-2 py-1 text-sm font-medium transition ${
												p === page ? "bg-[#030370] text-white" : "text-slate-700 hover:bg-slate-100"
											}`}
										>
											{p}
										</button>
				...
				<Modal title="Manual Check-in" onClose={() => setShowManual(false)}>
					<div className="space-y-4">
						<div>
							...
							<select
								value={manualEventId}
								onChange={(e) => { setManualEventId(e.target.value); setManualAttendeeId(""); }}
						...
						<div>
							<label className="mb-1 block text-sm font-medium text-slate-700">Attendee</label>
							<select
							...
							>
								<option value="">
									{!manualEventId ? "Select an event first" : attendeesQuery.isLoading ? "Loading..." : "Select attendee..."}
								</option>
						...
						<div className="flex justify-end gap-2 pt-2">
							...
							<button
								...
								disabled={!manualEventId || !manualAttendeeId || createCheckIn.isPending}
								className="rounded-lg bg-[#030370] px-4 py-2 text-sm font-semibold text-white hover:bg-[#0a4bb8] disabled:cursor-not-allowed disabled:opacity-50"
							>
			...
			{showScanner && (
				<Modal title="QR Code Check-in" onClose={() => setShowScanner(false)}>
					<div className="space-y-4">
						...
						<div>
							<label className="mb-1 block text-sm font-medium text-slate-700">Pass Code</label>
							<input
								type="text"
								value={qrCode}
								...
								className="w-full rounded-lg border border-slate-200 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0a4bb8]"
							/>
							<p className="mt-1 text-slate-500 text-xs">Enter the pass code from the attendee's QR ticket.</p>
						</div>
						<div className="flex justify-end gap-2 pt-2">
							<button
								type="button"
							...
							<button
								...
								disabled={!qrEventId || !qrCode.trim() || validatePass.isPending || createCheckIn.isPending}
								className="rounded-lg bg-[#030370] px-4 py-2 text-sm font-semibold text-white hover:bg-[#0a4bb8] disabled:cursor-not-allowed disabled:opacity-50"
							>
								{validatePass.isPending || createCheckIn.isPending ? "Processing..." : "Check In"}
							</button>
						</div>
					</div>
				</Modal>
...
function StatCard({ label, value }: { label: string; value: number }) {
	return (
		<div className="rounded-lg border border-[#dbe7ff] bg-white p-4">
			...
			<p className="mt-1 font-bold text-2xl text-slate-900">{value.toLocaleString("en-IN")}</p>
		</div>
	);
}
