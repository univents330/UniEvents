...
export function AuthScreen({ mode }: { mode: AuthMode }) {
	...
	const signupHref = authRedirectQuery
		? { pathname: "/auth/sign-up", query: authRedirectQuery }
		: "/auth/sign-up";
	...
	async function handleSubmit(event: FormEvent<HTMLFormElement>) {
		...
		try {
			if (mode === "login") {
				...
				if (response.error) {
					setSubmitError(response.error.message || "Unable to sign in.");
					return;
				}
			} else {
				...
				if (response.error) {
					setSubmitError(response.error.message || "Unable to create account.");
					return;
				}
			}
